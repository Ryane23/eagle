...
export default function UsersPage() {
    ...
    const { data: users = [], isLoading, error, refetch } = useUsersQuery({
        ...
        hospitalId: hospitalFilter === "all" ? undefined : hospitalFilter,
        status: statusFilter,
        search: searchQuery,
    });
    ...
    const groupedUsers = useMemo(() => {
        const groups = hospitals
            .map((hospital) => ({
                id: hospital.id,
                name: hospital.name,
                users: users.filter((user) => user.hospitalId === hospital.id),
            }))
            .filter((group) => group.users.length > 0);
        const unassignedUsers = users.filter(
            (user) => !user.hospitalId || !hospitalsById.has(user.hospitalId)
        );
    ...
    const handleCreateUser = useCallback(async () => {
        if (
            ...
            !formData.email ||
            !formData.password ||
            (formData.role !== "admin" && !formData.hospitalId)
        ) {
                                    ...
                                    groupedUsers.map((group) => (
                                        <Fragment key={group.id}>
                                            <TableRow className="bg-muted/40 hover:bg-muted/40">
                                                <TableCell colSpan={5} className="py-2 text-xs font-medium">
                                                    {group.name}
                                            ...
                                            {group.users.map((user) => (
                                                <TableRow key={user.id}>
                                                    ...
                                                    <TableCell className="text-sm text-muted-foreground">
                                                        {user.hospitalId
                                                            ? hospitalsById.get(user.hospitalId)?.name || "-"
                                                            : "-"}
                                                    </TableCell>
                                                    <TableCell>
                                                        <Badge
