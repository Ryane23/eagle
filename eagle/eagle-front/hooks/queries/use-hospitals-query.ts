...
export const hospitalKeys = {
    ...
    tree: () => [...hospitalKeys.all, "tree"] as const,
};
        ...
        queryFn: async () => {
            ...
            if (filters?.search) {
                ...
                hospitals = hospitals.filter(
                    (h) => h.name.toLowerCase().includes(search) || h.city?.toLowerCase().includes(search),
                );
            }
...
export function useHospitalTreeQuery() {
    return useQuery<Hospital[], Error>({
        queryKey: hospitalKeys.tree(),
        queryFn: getHospitalTree,
        staleTime: 60 * 1000,
        gcTime: 5 * 60 * 1000,
    });
}
...
export function useHospitalStats() {
    ...
    return {
        ...
        secondary: hospitals.filter((h) => h.type === "SUB").length,
    };
}
