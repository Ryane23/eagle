...
import type { SystemSettings, MaintenanceStatus, SystemSettingsHistory, UpdateSystemSettingsDto } from "@/types/api";
import {
    getSystemSettings,
...
export function useSystemSettingsHistoryQuery() {
    return useQuery<SystemSettingsHistory[], Error>({
        queryKey: systemKeys.history(),
        queryFn: getSystemSettingsHistory,
        staleTime: 30 * 1000,
    });
}
...
export function useUpdateSystemSettings() {
    ...
    return useMutation<SystemSettings, Error, UpdateSystemSettingsDto>({
        ...
        onSuccess: (updatedSettings) => {
            ...
            queryClient.invalidateQueries({ queryKey: systemKeys.history() });
            ...
            queryClient.setQueryData<SystemSettings>(
                ...
                (current) =>
                    current
                        ? {
                            ...current,
                            maintenanceMode: newStatus.isMaintenanceMode,
                        }
                        : current
            );
            toast.success(
