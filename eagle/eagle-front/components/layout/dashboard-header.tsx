...
export function DashboardHeader({ breadcrumbs, actions }: DashboardHeaderProps) {
    ...
    const adminStatsVisibility = useAdminStatsVisibility();
    ...
    const notificationsHref = isAdmin
        ...
        : pathname.startsWith("/dashboard/doctor")
            ...
            : pathname.startsWith("/dashboard/nurse")
                ? "/dashboard/nurse/notifications"
                : pathname.startsWith("/dashboard/primary")
                    ? "/dashboard/primary/notifications"
                    : "/dashboard/secondary/notifications";

    return (
        <header
            data-dashboard-header
            className={`flex shrink-0 items-center gap-2 border-b bg-background px-4 font-sans ${
                isAdmin ? "h-12" : "h-16"
            }`}
        >
            ...
            <div className="ml-auto flex items-center gap-4">
                {actions && (
                    <>
                        ...
                        <DropdownMenu>
                            ...
                            <DropdownMenuContent
                                align="end"
                                className="w-56 p-2 [&_button]:w-full [&_button]:justify-start [&>div]:flex-col"
                            >
                                {actions}
                            </DropdownMenuContent>
                        </DropdownMenu>
                ...
                {!isAdmin && (
                    <>
                        <div data-dashboard-search className="relative hidden lg:block">
                ...
                {isAdmin && adminStatsVisibility?.hasQuickStats && (
                    <Button
                        data-admin-stats-toggle
                        variant="ghost"
                        size="icon"
                        className="size-8"
                        onClick={adminStatsVisibility.toggleQuickStats}
                        aria-label={
                            adminStatsVisibility.statsHidden
                ...
                <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end" className="w-80">
                        ...
                        <DropdownMenuSeparator />
                        <DropdownMenuItem asChild className="justify-center text-accent">
