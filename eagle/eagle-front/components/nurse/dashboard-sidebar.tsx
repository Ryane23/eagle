...
function mapUrgencyToPatient(urgency: Urgency, currentTime: number): UrgentPatient {
    const createdAt = parseApiDate(urgency.createdAt);
    const waitTime = createdAt
        ? Math.max(0, Math.floor((currentTime - createdAt.getTime()) / 60000))
        : 0;
    return {
        id: urgency.id,
...
function mapConsultationToAppointment(
    ...
    return {
        ...
        specialty: consultation.specialtyId
            ? specialtyNames.get(consultation.specialtyId) || "Spécialité non renseignée"
            : "Médecine générale",
        urgencyLevel:
            consultation.urgencyLevel === "CRITICAL"
                ? 5
                : consultation.urgencyLevel === "URGENT"
                  ? 4
                  : consultation.urgencyLevel === "MODERATE"
                    ? 3
                    : 2,
        status,
    };
...
export function DashboardSidebar() {
    ...
    const todayAppointments = useMemo<Appointment[]>(() => {
        ...
        return schedule
            .filter((c) => {
                const consultDate =
                    parseApiDate(c.scheduledAt) || parseApiDate(c.createdAt);
                return consultDate?.toDateString() === today;
            })
            .map((consultation) =>
                mapConsultationToAppointment(consultation, specialtyNames),
            )
