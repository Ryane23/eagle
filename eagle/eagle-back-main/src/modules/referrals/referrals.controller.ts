...
export class ReferralsController {
  ...
  async create(@Body() createReferralDto: CreateReferralDto, @CurrentUser() user: User) {
    ...
    return await this.referralsService.create(createReferralDto, user);
  }
  ...
  async findByPatient(@Param('patientId') patientId: string, @CurrentUser() user: User) {
    return await this.referralsService.findByPatient(patientId, user);
  }

  @Get('status/:status')
  @Roles(UserRole.ADMIN, UserRole.PRIMARY_SECRETARY)
  @ApiOperation({ summary: 'Get referrals by status' })
  async findByStatus(@Param('status') status: ReferralStatus, @CurrentUser() user: User) {
    return await this.referralsService.findByStatus(status, user);
  }

  @Get(':id')
  @ApiOperation({ summary: 'Get referral by ID' })
  async findById(@Param('id') id: string, @CurrentUser() user: User) {
    return await this.referralsService.findById(id, user);
  }
  ...
  async update(
    ...
    return await this.referralsService.update(id, updateReferralDto, user);
  }
  ...
  async accept(
    ...
    return await this.referralsService.accept(id, user, acceptDto);
  }
  ...
  async reject(
    @Param('id') id: string,
    @Body() rejectDto: RejectReferralDto,
    @CurrentUser() user: User,
  ) {
    return await this.referralsService.reject(id, rejectDto, user);
  }

  @Patch(':id/in-transit')
  @Roles(UserRole.NURSE, UserRole.PRIMARY_SECRETARY)
  @ApiOperation({ summary: 'Mark referral as in transit' })
  async markInTransit(@Param('id') id: string, @CurrentUser() user: User) {
    return await this.referralsService.markInTransit(id, user);
  }
  ...
  async complete(@Param('id') id: string, @CurrentUser() user: User) {
    return await this.referralsService.complete(id, user);
  }
  ...
  async cancel(@Param('id') id: string, @CurrentUser() user: User) {
    return await this.referralsService.cancel(id, user);
  }
