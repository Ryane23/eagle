...
import { BranchStatistics, NetworkOverview } from './entities/analytics.entity';
import { UserCollection, UserRole } from '../users/entities/user.entity';
import {
  HospitalCollection,
  HospitalType,
  Hospital,
...
import {
  Urgency,
  UrgencyCollection,
} from '../urgencies/entities/urgency.entity';
...
export class AnalyticsService {
  ...
  async getNetworkOverview(): Promise<NetworkOverview> {
    ...
    const primaryCenters = hospitals.filter(
      (h) => h.type === HospitalType.PRIMARY,
    ).length;
    const secondaryCenters = hospitals.filter(
      (h) => h.type === HospitalType.SUB,
    ).length;
    ...
    const networkWaitTimes = queueEntries
      .map((q) => q.estimatedWaitMinutes)
      .filter((minutes): minutes is number => Number.isFinite(minutes));
    const averageWaitTime =
      networkWaitTimes.length > 0
        ? Math.round(
            networkWaitTimes.reduce((sum, minutes) => sum + minutes, 0) /
              networkWaitTimes.length,
          )
        : 0;

    // Get urgencies
    const urgenciesSnapshot = await this.firebaseService
      .collection(UrgencyCollection)
      .get();
    const urgencies = urgenciesSnapshot.docs.map((doc) => ({
      id: doc.id,
      ...doc.data(),
      createdAt: doc.data().createdAt?.toDate?.() || doc.data().createdAt,
    })) as Urgency[];
    const totalUrgencies = urgencies.length;
    const urgenciesToday = urgencies.filter((urgency) => {
      const created =
        urgency.createdAt instanceof Date
          ? urgency.createdAt
          : new Date(urgency.createdAt);
      return created >= startOfToday;
    }).length;
  ...
  async getBranchStatistics(hospitalId: string): Promise<BranchStatistics> {
    ...
    const usersByRole = {
      ...
      primary_secretary: users.filter(
        (u) => u.role === UserRole.PRIMARY_SECRETARY,
      ).length,
      secondary_secretary: users.filter(
        (u) => u.role === UserRole.SECONDARY_SECRETARY,
      ).length,
      ...
      specialist:
        hospital.type === HospitalType.PRIMARY
          ? users.filter(
              (u) => u.role === UserRole.DOCTOR && Boolean(u.specialtyId),
            ).length
          : 0,
    };

    // Load patients so ownership can be resolved directly from hospitalId.
    const patientsSnapshot = await this.firebaseService
      .collection(PatientCollection)
      .get();
    ...
    const consultationsByStatus = {
      scheduled: hospitalConsultations.filter(
        (c) => c.status === ConsultationStatus.SCHEDULED,
      ).length,
      in_progress: hospitalConsultations.filter(
        (c) => c.status === ConsultationStatus.IN_PROGRESS,
      ).length,
      completed: hospitalConsultations.filter(
        (c) => c.status === ConsultationStatus.COMPLETED,
      ).length,
      cancelled: hospitalConsultations.filter(
        (c) => c.status === ConsultationStatus.CANCELLED,
      ).length,
    };
    const consultationsByType = {
      video: hospitalConsultations.filter(
        (c) => c.type === ConsultationType.VIDEO,
      ).length,
      audio: hospitalConsultations.filter(
        (c) => c.type === ConsultationType.AUDIO,
      ).length,
      chat: hospitalConsultations.filter(
        (c) => c.type === ConsultationType.CHAT,
      ).length,
    };
    ...
    if (completedConsultations.length > 0) {
      const totalDuration = completedConsultations.reduce((sum, c) => {
        const start =
          c.startedAt instanceof Date ? c.startedAt : new Date(c.startedAt);
        const end = c.endedAt instanceof Date ? c.endedAt : new Date(c.endedAt);
        return sum + (end.getTime() - start.getTime());
      }, 0);
      averageConsultationDuration = Math.round(
        totalDuration / completedConsultations.length / 60000,
      ); // in minutes
    }
    ...
    const hospitalQueueEntries = allQueueEntries.filter(
      (q) =>
    ...
    const queueByStatus = {
      waiting: hospitalQueueEntries.filter(
        (q) => q.status === QueueStatus.WAITING,
      ).length,
      in_progress: hospitalQueueEntries.filter(
        (q) => q.status === QueueStatus.IN_PROGRESS,
      ).length,
      completed: hospitalQueueEntries.filter(
        (q) => q.status === QueueStatus.COMPLETED,
      ).length,
      cancelled: hospitalQueueEntries.filter(
        (q) => q.status === QueueStatus.CANCELLED,
      ).length,
    };
    const waitTimes = hospitalQueueEntries
      .map((q) => q.estimatedWaitMinutes)
      .filter((minutes): minutes is number => Number.isFinite(minutes));
    const averageWaitTime =
      waitTimes.length > 0
        ? Math.round(
            waitTimes.reduce((sum, minutes) => sum + minutes, 0) /
              waitTimes.length,
          )
        : 0;

    // Get referrals sent or received by this hospital.
    const referralsSnapshot = await this.firebaseService
      .collection(ReferralCollection)
      .get();
    const hospitalReferrals = referralsSnapshot.docs
      .map((doc) => ({ id: doc.id, ...doc.data() }) as Referral)
      .filter(
        (referral) =>
          referral.fromHospitalId === hospitalId ||
          referral.toHospitalId === hospitalId,
      );
    const totalReferrals = hospitalReferrals.length;
    const pendingReferrals = hospitalReferrals.filter(
    ...
    // Patients belong to the hospital where they were registered.
    ...
    const activePatients = hospitalPatients.filter(
      (p) => p.isActive === true,
    ).length;
