...
import { AuthToken, AuthTokenCollection } from './entities/auth-token.entity';
...
export class AuthService {
  ...
  async getContext(user: User) {
    ...
    const [parentHospital, departmentDoc, shiftSnapshot, boxSnapshot] =
      await Promise.all([
        hospital.parentHospitalId
          ? this.hospitalsService.findById(hospital.parentHospitalId)
    ...
    return {
      ...
      parentHospital,
      department:
        departmentDoc && departmentDoc.exists
          ? { id: departmentDoc.id, ...departmentDoc.data() }
          : null,
      activeShift: shiftSnapshot.empty
        ? null
        : {
