...
export class FollowupsService {
  ...
  async findAll(
    ...
    if (userRole === UserRole.DOCTOR && userId) {
      ...
      if (!this.firebaseService) return followups;
      const firebaseService = this.firebaseService;
      const scoped = await Promise.all(
        followups.map(async (followup) => {
          const patient = await firebaseService
            .collection('patients')
            .doc(followup.patientId)
            .get();
          return patient.data()?.hospitalId === userHospitalId ? followup : null;
        }),
      );
      followups = scoped.filter((item): item is Followup => !!item);
    } else {
