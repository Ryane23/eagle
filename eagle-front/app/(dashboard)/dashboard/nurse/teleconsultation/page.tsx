...
export default function TeleconsultationPage() {
    ...
    const teleconsultations = useMemo(
        () => apiConsultations.filter((c) => c.type === "video").map(mapConsultationToTeleconsultation),
        [apiConsultations]
    );
    ...
    // Auto-select active consultation when data loads
    useEffect(() => {
        if (teleconsultations.length > 0 && !selectedConsultation) {
            const active = teleconsultations.find(t => t.status === "active");
            if (active) setSelectedConsultation(active);
        }
    }, [teleconsultations, selectedConsultation]);
