...
import { useConsultationsQuery } from "@/hooks/queries/use-consultations-query";
import { 
...
export default function NurseMessagingPage() {
    ...
    const contacts = useMemo(() => {
        ...
        return users
            .filter(u => ["DOCTOR", "NURSE", "SECRETARY", "PRIMARY_DOCTOR"].includes(u.role))
            .map(user => {
    ...
    } = useConsultationMessagesQuery(selectedContact?.consultationId || "");
    ...
    const handleSendMessage = () => {
        if (!messageText.trim() || !selectedContact?.consultationId) {
            if (!selectedContact?.consultationId) {
                toast.error("Aucune consultation active pour ce contact");
            }
            return;
        }
                            ...
                            <div className="p-3 border-t shrink-0">
                                {selectedContact.consultationId ? (
                                    ...
                                    <div className="text-center py-4 text-muted-foreground text-sm">
                                        ...
                                        Aucune consultation active pour envoyer des messages
                                    </div>
                                )}
                            </div>
                        </>
