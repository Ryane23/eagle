...
function makeSnapshot<T extends object>(docs: Array<FirestoreDoc<T>>): FirestoreQuerySnapshot<T> {
...
describe('AuthService.login', () => {
  ...
  it('throws UnauthorizedException when user is not found', async () => {
    (firebaseService.collection as jest.Mock).mockImplementation((name: string) => {
    ...
    await expect(
      authService.login({ email: 'missing@eagles.com', password: 'Password@123' }),
  ...
  it('throws UnauthorizedException when user is inactive', async () => {
    ...
    await expect(
      authService.login({ email: 'doctor.nana@eagles.com', password: 'Doctor@123' }),
    ).rejects.toBeInstanceOf(UnauthorizedException);
  });
  ...
  it('throws UnauthorizedException when password is invalid', async () => {
    ...
    await expect(
      authService.login({ email: 'doctor.nana@eagles.com', password: 'Wrong@123' }),
    ).rejects.toBeInstanceOf(UnauthorizedException);
  });
